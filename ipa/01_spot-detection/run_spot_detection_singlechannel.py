...
def detect_spots(img_slice: ArrayLike, mask: ArrayLike, wavelength: int, NA: float, spacing: tuple[float, float], k: float):
    """Spot detection with LoG filter and h-maxima and std as threshold."""

    sigma = wavelength/ ( 2 * NA ) / np.sqrt(2) / (spacing[1] * 1000)
    log_img = -gaussian_laplace(img_as_float32(img_slice), sigma=sigma) * sigma**2
    log_img = img_as_uint(
        np.clip(
            normalize_minmse(log_img, img_as_float32(img_slice)),
            -1,
            1
        )
    )
...
def refine_spots(spots_per_roi: list, roi_labels: list, img_slice: ArrayLike, frame: int, logger: logging.Logger):
    subpix_spots = {}
    for roi_id, (y_coords, x_coords) in zip(roi_labels, spots_per_roi):
        subpix_spots[roi_id] = []
        for y, x in zip(y_coords, x_coords):
            spot_img, start_y, start_x = get_spot(img_slice, [y, x], size=5, logger=logger)
            try:
...
def get_raw_spot_intensity_computer(
    ...
    def raw_spot_intensity_computer(row):
        ...
        sum_intensity = np.mean(img[y-1:y+1, x-1:x+1])
...
def detect_spots_in_frame(img_slice: ArrayLike, mask: ArrayLike, frame: int,
    ...
    try:
    	spots_for_frame_df['mean_spot_intensity'] = spots_for_frame_df.apply(get_raw_spot_intensity_computer(img_slice), axis=1)
...
def detect_spots_in_2DTime(img_file: str, mask_file: str,
    ...
    futures = []
    progress = tqdm(total=img.shape[0], smoothing=0.0, leave=False)
    pool = Pool(8)
