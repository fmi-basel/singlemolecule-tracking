...
def detect_spots_in_frame(denoised_slice: ArrayLike, raw_slice: ArrayLike, mask: ArrayLike, actin_slice: ArrayLike, tubulin_slice: ArrayLike, frame: int,
    ...
    try:
        intensity_computer = get_raw_spot_intensity_computer(raw_slice, actin_slice, tubulin_slice)
...
def detect_spots_in_2DTime(img_file: str, mask_file: str, actin_file: str, tubulin_file: str,
                           NA: float, wavelength: int, spacing: tuple[float, float], k:float):
...
if __name__ == "__main__":
    ...
    for img_file in tqdm(img_files):
        ...
        actin_file = join(config['actin_file'], basename(img_file).replace(".tif", "_actin.tif"))
        tubulin_file = join(config['tubulin_file'], basename(img_file).replace(".tif", "_tubulin.tif"))
