...
def get_spot(img, coords, size=21, logger: logging.Logger = logging.getLogger()):
    ...
    def _warn():
        logger.warning(
            f"Spot {coords} is close to image boundary. Spot detection might be inaccuarte."
        )
...
def subpixel_localization_2d(spot_img, spacing):
    ...
    bounds = [
        (
            0,
            init_params[1] * 0.5,
            init_params[2] - 3 * spacing[1],
            init_params[3] - 3 * spacing[0],
            -3,
            -3,
        ),
        (
    ...
    try:
        popt, pcov = curve_fit(
            eval_gauss_2d, coords_yx, spot_img.ravel(), p0=init_params, bounds=bounds
        )
    except ValueError:
